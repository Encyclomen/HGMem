...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    if "max_tokens" not in kwargs:
        kwargs["max_tokens"] = kwargs.pop("max_new_tokens", None)
...
async def vllm_complete_if_cache(
    ...
    """
    Complete function for vLLM deployed models using OpenAI-compatible API.
    ...
    Args:
        ...
        api_key: API key if required (can be None for local vLLM)
        **kwargs: Additional parameters for the API call

    Returns:
        Model response as string or async iterator for streaming
    """
    ...
    vllm_client = AsyncOpenAI(**client_kwargs)
    ...
    # Add debug logging
    logger.debug("===== Query Input to vLLM =====")
    logger.debug(f"Query: {prompt}")
    logger.debug(f"System prompt: {system_prompt}")
    logger.debug("Full context:")
    for msg in messages:
...
async def hf_model_if_cache(
    ...
    **kwargs,
    ...
    output = hf_model.generate(
        **input_ids, max_new_tokens=max_new_tokens, num_return_sequences=1, early_stopping=True
    )
...
async def vllm_complete(
    ...
    """
    Complete function for vLLM models with keyword extraction support.
    """
    keyword_extraction = kwargs.pop("keyword_extraction", None)
    if keyword_extraction:
        kwargs["response_format"] = GPTKeywordExtractionFormat

    # Get model configuration from hashing_kv if available
